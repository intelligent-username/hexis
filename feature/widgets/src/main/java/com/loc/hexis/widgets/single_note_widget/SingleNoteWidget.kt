...
private fun SingleNoteContent(
    ...
    val openAction =
        actionStartActivity(
            AppIntents.openMain(
                    context,
                    if (note != null) WidgetActions.OPEN_NOTE else WidgetActions.OPEN_NOTES,
                )
                .apply {
                    if (note != null) putExtra("note_id", note.id)
                }
        )
        ...
        if (note == null) {
            ...
            when (note.type) {
                NoteType.MARKDOWN -> {
                    val lines =
                        note.content.let {
                            if (it.isBlank()) emptyList() else parseContentLines(it)
                        }
                    if (lines.isEmpty()) {
                        Box(
                            modifier =
                            ...
                            Text(
                                text = "No content",
                                style =
                                    TextStyle(fontSize = 12.sp, color = cardColors.onSurfaceVariant),
                            )
                        }
                    } else {
                        LazyColumn(
                            ...
                            items(lines) { line ->
                                when (line.type) {
                                    ...
                                    LineType.CHECKLIST ->
                                        Text(
                                            text =
                                                "${if (line.isChecked) "☑" else "☐"} ${line.text}",
                                            style =
                                                TextStyle(
                                                    fontSize = 12.sp,
                                                    color = cardColors.onSurfaceVariant,
                    ...
                    if (tableData.rows.isEmpty()) {
                        ...
                        LazyColumn(modifier = GlanceModifier.fillMaxSize()) {
                            items(
                                tableData.rows,
                                itemId = { row ->
                                    (row.id + "_" + row.label + "_" + row.value + "_" + row.step)
                                        .hashCode()
                                        .toLong()
                                },
                            ) { row ->
                    ...
                    if (journalData.entries.isEmpty()) {
                        ...
                        LazyColumn(modifier = GlanceModifier.fillMaxSize()) {
                            items(
                                journalData.entries,
                                itemId = { entry ->
                                    (entry.id + "_" + entry.timestamp + "_" + entry.text)
                                        .hashCode()
                                        .toLong()
                                },
                            ) { entry ->
...
private fun CounterWidgetRow(
    ...
    val valText =
        if (row.value % 1.0 == 0.0) row.value.toLong().toString() else row.value.toString()
